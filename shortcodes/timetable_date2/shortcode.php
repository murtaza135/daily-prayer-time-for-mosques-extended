...
function dpte_timetable_date2_shortcode($atts) {
  wp_enqueue_style("dpte_timetable_date2", plugin_dir_url(__FILE__) . "styles.css");
  wp_enqueue_script("dpte_timetable_date2", plugin_dir_url(__FILE__) . "script.js", ["dpte_dpt_cache", "dpte_date_time_utils"], null, true);
  ...
  $atts = shortcode_atts($default_atts, $atts, 'dpte_timetable_date2');
  ...
  ob_start();
  ?>
  <div class="dpte-timetable-date2" style="<?php echo esc_attr($style); ?>" <?php echo $data_attrs; ?>>
    <p class="dpte-timetable-date2-text" <?php echo $data_attrs; ?>><?php echo date("jS F Y") ?></p>
  </div>
  <?php
  return ob_get_clean();
}

add_shortcode('dpte_timetable_date2', 'dpte_timetable_date2_shortcode');
